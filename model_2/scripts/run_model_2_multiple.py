...
for k in range(n_runs):
    ...
    solve_script = Path(__file__).resolve().parent / "solve_model_2.py"
    ...
    run_result["covered_signature"] = ",".join(map(str, run_result["covered_ids"]))
    all_runs.append(run_result)

df = pd.DataFrame(all_runs)
df.to_csv(results / "model_2_multiple_runs.csv", index=False)

same_objective = df["objective"].nunique() == 1
same_bases = df["bases"].nunique() == 1
same_solution = df["solution_signature"].nunique() == 1
same_covered = df["covered_signature"].nunique() == 1
...
summary = {
    ...
    "covered_points_unique_values": sorted(df["covered_points"].dropna().unique().tolist()),
    ...
    "same_exact_covered_set_all_runs": same_covered,
...
with open(results / "model_2_multiple_runs_summary.json", "w") as f:
